# Japan Trip Plan (Latest) — 4 Adults
*(Apr 11–17; hotels booked; food-heavy + second-hand luxury; Ghibli + Kyoto day trip + Narita Omotesando)*

> **Updated with official-source checks as of Jan 2026.** Timetables/prices can change—confirm day-of in your map app / station signs.

---

## Key Assumptions / Open Questions (Fill These In)
- **Flights**: arrival time to NRT on **Apr 11** + departure time from NRT on **Apr 17** (drives buffer + when to leave hotel).
- **Phones**: confirm each adult has an **iPhone that supports Suica**; if not, plan **physical IC card + cash**.
- **Who is the “3 people” on Apr 16?** Name who goes to Omotesando vs who rests at hotel.
- **Luggage**: decide if you’ll **forward bags** (Tokyo→Osaka and/or Osaka→Narita) or all carry yourselves.
- **Shopping targets**: list 5–10 “must-check” resale stores/areas in Shinjuku + Ginza (so you don’t wander).

---

## Hotels (Booked)
- **Apr 11–13:** Keio Plaza Hotel Tokyo (Shinjuku)
- **Apr 13–16:** Courtyard Shin-Osaka Station (Osaka base)
- **Apr 16–17:** ANA Crowne Plaza Narita (Narita overnight)

---

## Transportation & Payments (Quick)
### IC Card
- Each person uses **Suica on iPhone (Apple Wallet)** for local trains/subways/buses (Tokyo/Osaka/Kyoto).  
  - If anyone can’t do mobile Suica: plan **physical IC card** (and keep **¥10,000–¥20,000 cash** as backup).
- **Shinkansen + Narita Express (N’EX)**: buy/reserve separately (not just a Suica tap).
- **Cards**: some ticket sites/apps can be picky with foreign cards; have **2 different credit cards** available if possible.

### Typical “Avg wait” assumptions (for planning)
- Local trains (Tokyo/Osaka): **~5–7 min**
- N’EX: **~15–20 min**
- Shinkansen: **~10–15 min**
- Taxi queue: **~5 min**

### Taxi vs Train (Cost for 4 people)
- **General Rule**: For short hops (<5km), Taxi is ~¥1,500–¥2,500. Split by 4, it's ~¥400–¥600/person (comparable to or slightly more than train, but saves walking).
- **Keio Plaza → Shinagawa Station**: ~¥3,500–¥4,500 total (~¥1,000/person). Saves hauling luggage on Yamanote line.

---

## Itinerary (with transportation + duration estimates)

### **Fri Apr 11 — Arrive Narita → Meet → Shinjuku (Keio Plaza)**
**Plan to meet and go together**
1) **Meet at NRT Terminal 2 Arrivals Hall** (choose one landmark: info counter / main arrivals exit).
2) Go to **Airport Limousine Bus Counter** (Arrivals floor).
3) Buy tickets for **Keio Plaza Hotel / Shinjuku Area**.
4) Board bus at designated stop (staff will handle luggage).
5) Arrive **Keio Plaza Hotel** (Door-to-door drop off).

**Transit durations (ride + avg wait)**
- Limousine Bus (NRT → Keio Plaza): **~85–110 min** (traffic dependent)
- Walking: **0 min** (direct drop)
**Transit total:** ~**2h** (relaxed, no hauling bags)

**Notes**
- **Limousine Bus** is much easier than trains with luggage. No navigating huge stations.
- Add a **realistic arrival buffer**: immigration + bags + regrouping can be **60–120 min**.

---

### **Sat Apr 12 — Ghibli 10:00 (≤2 hrs) + Kichijoji lunch + Shinjuku + Ginza**
#### **To Ghibli (train + taxi mix)**
- **8:15** Leave hotel → Shibuya Station
  - **Taxi option**: ~20 min direct from Keio Plaza to Shibuya Station (~¥2,500 total / ¥625 pp). Saves Shinjuku station walk/train.
  - **Train option**: Walk to Shinjuku Stn + Yamanote Line (~¥170 pp).
- **8:30** **Keio Inokashira Line** Shibuya → **Kichijoji**
- **~8:50** Arrive Kichijoji → go to **Taxi stand** (follow “タクシー”)
- Taxi Kichijoji → **Ghibli Museum**
- Arrive **~9:25–9:40** (buffer for 10:00 entry)

**Ghibli Museum**
- **10:00–12:00** (max 2 hours)

#### **From Ghibli (taxi + train)**
- **12:00–12:15** Taxi Ghibli → **Kichijoji Station**
- **12:15–1:15** Lunch in **Kichijoji** *(decide later)*
- **1:15–1:40** JR Chuo Line **Kichijoji → Shinjuku**
- **1:45–4:30** Shinjuku second-hand luxury (big inventory / compare prices)
- **4:30–5:00** Tokyo Metro **Shinjuku → Ginza**
- **5:00–7:00** Ginza second-hand luxury (curated / premium browsing)
- **7:00–9:00** Dinner in Ginza
- **9:00–9:30** Ginza → Shibuya (JR/Metro)

**Transit durations (ride + avg wait, across the day)**
- Inokashira Line: **~18 min + ~6 wait**
- Taxi Kichijoji→Ghibli: **~10 min + ~5 wait**
- Taxi Ghibli→Kichijoji: **~10 min + ~5 wait**
- JR Chuo: **~16 min + ~6 wait**
- Metro Shinjuku→Ginza: **~20 min + ~5 wait**
- Metro/JR Ginza→Shibuya: **~13–20 min + ~5 wait**
**Transit total (across day):** ~**2h+** (not incl. museum/shopping/meals)

**Notes**
- If taxi lines are long: consider **bus/walk** options from **Mitaka/Kichijoji** as backup (decide on the day).
- Shinjuku/Ginza luxury resale is easier if you pre-pick a few targets; otherwise set a **hard stop time** so dinner doesn’t slide.

---

### **Sun Apr 13 — Tokyo → Shin-Osaka (Courtyard) + Namba/Dotonbori**
1) Move: **Shinjuku → Shinagawa Station**
   - **Option A (Train)**: Walk to Shinjuku Stn + JR Yamanote Line (~40 min total, ~¥200 pp). Crowded with luggage.
   - **Option B (Taxi)**: Door-to-door from Keio Plaza → Shinagawa Stn (~25–35 min, ~¥3,500–¥4,500 total / ~¥1,000 pp). **Recommended for 4 pax + luggage.**
2) Shinkansen: **Shinagawa → Shin-Osaka**
3) Walk to hotel (station-adjacent)
4) Osaka Metro Midosuji: **Shin-Osaka → Namba** for food night

**Transit durations**
- JR Shinjuku→Shinagawa: **~20 min + ~5 wait**
- Shinkansen Shinagawa→Shin-Osaka: **~140 min + ~15 wait**
- Midosuji Shin-Osaka→Namba: **~15 min + ~5 wait**
**Transit total:** ~**3h 20m**

**Notes**
- Decide whether to reserve seats together vs buy day-of. With 4 adults + luggage, **reserved seats** reduce hassle.
- If you forward luggage, this day becomes much easier (carry only an overnight bag on the train).

---

### **Mon Apr 14 — Nara Day Trip**
**Route (best for Nara Park side)**
1) Midosuji: **Shin-Osaka → Namba**
2) Kintetsu: **Osaka-Namba → Kintetsu-Nara**
3) Return same way

**Transit durations (round trip)**
- Midosuji: **~15 + ~5 wait** each way
- Kintetsu: **~40 + ~10 wait** each way
**Transit total:** ~**2h 20m** (excluding time in Nara)

**Notes**
- Kintetsu-Nara is walkable to Nara Park/Todaiji; it’s a good choice for the “park side” sights.
- Add 1–2 “anchors” so the day has shape (e.g., **Todaiji → Nara Park deer → Mochi/pastry stop**).

---

### **Tue Apr 15 — Kyoto Day Trip (Your Must-Visits)**
**Order (efficient + crowd-smart):** **Fushimi Inari → Gion → Nishiki Market**

1) Shinkansen: **Shin-Osaka → Kyoto**
2) Move: **Kyoto Station → Inari Station** (Fushimi Inari)
   - **Option A (Train)**: JR Nara Line (~5 min ride + walk, ¥150 pp). Fast/cheap.
   - **Option B (Taxi)**: Kyoto Stn Hachijo Exit → Inari (~10–15 min, ~¥1,500 total / ¥375 pp). Drops closer to main gate.
3) Move **Inari → Gion**
   - Option A (rail): **Keihan** + short walks
   - Option B (taxi): fastest / simplest for 4 people
4) Move **Gion → Nishiki Market** (walk/taxi/bus depending on where you are)
5) Return: **Kyoto → Shin-Osaka** (Shinkansen)

**Transit durations (typical)**
- Shinkansen Shin-Osaka→Kyoto: **~15 min + ~10 wait**
- JR Kyoto→Inari: **~5 min + ~7 wait**
- Inari→Gion:
  - Rail option: **~12 min + ~6 wait** (+ walking)
  - Taxi option: **~20–30 min + ~5 wait**
- Return Kyoto→Shin-Osaka: **~15 min + ~10 wait**
**Transit total:** ~**1h 05m–1h 30m** (excluding sightseeing)

**Notes**
- Fushimi Inari is best **early**; consider an “arrive Inari before ~9:00” target if you want lighter crowds.
- For Shin-Osaka→Kyoto, Shinkansen is fastest, but there’s also a slower/cheaper **JR Kyoto Line** fallback if timing works.

---

### **Wed Apr 16 — Osaka → Narita + Drop Luggage + Naritasan Omotesando (3 people)**
1) Shinkansen: **Shin-Osaka → Shinagawa**
2) N’EX: **Shinagawa → Narita Airport**
3) Hotel shuttle: **NRT → ANA Crowne Plaza Narita**
   - **Alternative**: Taxi (~10 min, ~¥2,000) if you miss the shuttle.
4) Drop luggage / check in
5) Hotel shuttle: **Hotel → Narita Station (Downtown)**
6) Walk **Narita Omotesando → Naritasan Shinshoji**
7) Return to hotel (shuttle back or taxi)

**Transit durations (ride + avg wait)**
- Shinkansen Shin-Osaka→Shinagawa: **~140 min + ~15 wait**
- N’EX Shinagawa→NRT: **~75 min + ~20 wait**
- Hotel shuttle NRT→hotel: **~15–25 min + ~10–20 wait**
- Downtown shuttle hotel↔Narita Station (if used): **~15–25 min + ~10–20 wait** (each way)
**Transit total to hotel (from Osaka):** ~**4h** (excluding airport time)  
**Add Omotesando outing transit:** ~**1h** round trip (plus walking + dinner)

**Notes**
- Confirm **shuttle timetable** (airport ↔ hotel ↔ downtown) so you don’t strand the group in Narita.
- Add a simple plan for the 4th person if only 3 go out (rest, onsen/relax, or meet later for dinner).

---

### **Thu Apr 17 — ANA Crowne Plaza Narita → Narita Airport**
- Hotel shuttle back to airport (or Taxi ~¥2,000 for convenience)

**Transit durations**
- Shuttle: **~15–25 min + ~10–20 wait**
**Transit total:** ~**30–45 min** (excluding airport procedures)

---

## What to Reserve in Advance (with links)
> **Links are in code blocks** so you can copy/paste easily.

### 0) Visit Japan Web (immigration + customs QR)
```text
https://services.digital.go.jp/en/visit-japan-web/
```

### 1) Ghibli Museum (timed entry; advance reservation only)
- Official page says **all admission is by advance reservation only**; **no tickets sold at the museum**.
- Official page says overseas tickets are sold via **LAWSON TICKET** and go on sale **10:00 a.m. (JST) on the 10th** for the **following month**.
```text
https://www.ghibli-museum.jp/en/tickets/

# Lawson Ticket (English) entry (from the official Ghibli tickets page)
https://l-tike.com/st1/ghibli-en/Tt/Ttg010agreement/index
```

### 2) Shinkansen reservations (Tokyo↔Shin-Osaka + Kyoto day + Shin-Osaka→Tokyo area)
**SmartEX (recommended)**
```text
https://smart-ex.jp/en/
```

### 3) Airport Limousine Bus (NRT → Keio Plaza)
- **No reservation needed usually**, but can buy at airport counter.
- If you want to book ahead (optional):
```text
https://webservice.limousinebus.co.jp/web/en/
```
### 4) Kintetsu Limited Express (optional)
*(Only if you choose Limited Express; regular Kintetsu trains don’t require reservations.)*
- Kintetsu’s site notes the online purchase is for the **Limited Express ticket**; you also need the **base fare ticket** separately.
```text
https://www.ticket.kintetsu.co.jp/vs/en/T/TZZ/TZZ10.do?op=tDisplayVisitorMenu
```

---

## “Day-Of” Checklist (Quick)
- **Before leaving each hotel**: confirm checkout time, store luggage if needed, and screenshot the hotel address in Japanese.
- **Before each long train** (Shinkansen / N’EX): confirm departure platform + car number + seat numbers.
- **Cash / cards**: keep some cash for small shops/taxis; have at least two cards between the group.
- **Group rule**: pick one “navigator” and one “pay person” per transit leg to reduce confusion.

## Google Maps Quick Links (copy/paste)
```text
Keio Plaza Hotel Tokyo:
https://www.google.com/maps/search/?api=1&query=Keio+Plaza+Hotel+Tokyo

Ghibli Museum:
https://www.google.com/maps/search/?api=1&query=Ghibli+Museum+Mitaka

Kichijoji Station:
https://www.google.com/maps/search/?api=1&query=Kichijoji+Station

Shinjuku (luxury resale area):
https://www.google.com/maps/search/?api=1&query=Shinjuku+second+hand+luxury

Ginza (luxury resale area):
https://www.google.com/maps/search/?api=1&query=Ginza+second+hand+luxury

Courtyard Shin-Osaka Station:
https://www.google.com/maps/search/?api=1&query=Courtyard+Shin-Osaka+Station

Dotonbori:
https://www.google.com/maps/search/?api=1&query=Dotonbori

Kintetsu-Nara Station:
https://www.google.com/maps/search/?api=1&query=Kintetsu-Nara+Station

Fushimi Inari Taisha:
https://www.google.com/maps/search/?api=1&query=Fushimi+Inari+Taisha

Gion, Kyoto:
https://www.google.com/maps/search/?api=1&query=Gion+Kyoto

Nishiki Market:
https://www.google.com/maps/search/?api=1&query=Nishiki+Market

ANA Crowne Plaza Narita:
https://www.google.com/maps/search/?api=1&query=ANA+Crowne+Plaza+Narita

Narita Omotesando:
https://www.google.com/maps/search/?api=1&query=Naritasan+Omotesando

Naritasan Shinshoji:
https://www.google.com/maps/search/?api=1&query=Naritasan+Shinshoji
```

---

## YouTube “Visualize the Ride” Links (search links)
```text
Airport Limousine Bus Narita to Hotel:
https://www.youtube.com/results?search_query=Airport+Limousine+Bus+Narita+to+Shinjuku

JR Chuo Line Rapid Shinjuku to Kichijoji:
https://www.youtube.com/results?search_query=JR+Chuo+Line+Rapid+Shinjuku+to+Kichijoji

Kichijoji taxi stand (Japanese):
https://www.youtube.com/results?search_query=%E5%90%89%E7%A5%A5%E5%AF%BA%E9%A7%85+%E3%82%BF%E3%82%AF%E3%82%B7%E3%83%BC+%E4%B9%97%E3%82%8A%E5%A0%B4

Shinkansen Tokyo/Shinagawa to Shin-Osaka full ride:
https://www.youtube.com/results?search_query=Shinkansen+Shinagawa+to+Shin-Osaka+Nozomi+ride

Osaka Metro Midosuji Line Shin-Osaka to Namba:
https://www.youtube.com/results?search_query=Osaka+Metro+Midosuji+Line+Shin-Osaka+to+Namba

Kintetsu Osaka-Namba to Nara:
https://www.youtube.com/results?search_query=Kintetsu+Osaka+Namba+to+Nara+train+ride

Shin-Osaka to Kyoto Shinkansen:
https://www.youtube.com/results?search_query=Shin-Osaka+to+Kyoto+Shinkansen+ride

Kyoto Station to Fushimi Inari JR Nara Line:
https://www.youtube.com/results?search_query=Kyoto+Station+to+Inari+Station+JR+Nara+Line

Narita Omotesando walking:
https://www.youtube.com/results?search_query=Naritasan+Omotesando+walk
```

---

## Notes / Decisions You Can Make Later
- **Kichijoji lunch (Apr 12):** decide on the day (you’ll have ~12:15–1:15).
- **Inari → Gion transport (Apr 15):** rail if you prefer cheap; taxi if you prefer speed/comfort for 4.

---
